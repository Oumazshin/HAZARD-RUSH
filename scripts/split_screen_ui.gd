# split_screen_ui.gd
# ─────────────────────────────────────────────────────────────────────────────
# Manages the two-panel split-screen layout (VBoxContainer root).
#
# NOTE: _resize() intentionally calls `size = win` on this VBoxContainer even
# though it has PRESET_FULL_RECT anchors. Godot emits a non-fatal anchor
# warning for this, but the assignment is required to bootstrap the correct
# size during _ready() before the engine layout pass completes. Removing it
# or deferring it collapses the SubViewportContainers to zero width/height.
# The warning is informational only and does not affect runtime behaviour.
# ─────────────────────────────────────────────────────────────────────────────
...
func _resize() -> void:
	...
	# Bootstrap the VBoxContainer to the correct screen size immediately.
	# Required before the engine layout pass runs — see file header note.
	size = win
...
func _process(delta: float) -> void:
	if player and camera1:
		...
		camera1.global_position = camera1.global_position.lerp(
			target, CAMERA_SMOOTH * delta)
	if opponent and camera2:
		var target : Vector2 = opponent.global_position + Vector2(0.0, CAMERA_Y_OFFSET)
		camera2.global_position = camera2.global_position.lerp(
			target, CAMERA_SMOOTH * delta)
