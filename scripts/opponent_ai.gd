...
func _ready() -> void:
	...
	# Get the SabotageSystem node from GameWorld/PlayerLane
	sabotage_system = get_parent().get_parent().get_node_or_null("PlayerLane/SabotageSystem")
	if not sabotage_system:
		push_warning("[OpponentAI] SabotageSystem node not found in GameWorld.")
...
func _evaluate_sabotage() -> void:
	...
	if decision == "ACTIVATE":
		if sabotage_system:
			sabotage_system.trigger()
		sabotage_cooldown = sabotage_cooldown_time
		print("[OpponentAI] Sabotage activated!")
...
func _trigger_stumble() -> void:
	...
	_disable_slide_shapes()
	current_speed *= 0.3
	GameState.apply_kei_penalty("ai", "HIGH_HURDLE")
