...
var player_distance       : float = 0.0 # max x-position reached during the race
...
var ai_distance            : float = 0.0 # max x-position reached during the race
...
#  ai_greedy_count       → opponent_ai.gd,         inside _trigger_evasion()
...
func _process(delta: float) -> void:
	...
	# Stumble timers were set by apply_kei_penalty() but never decremented,
